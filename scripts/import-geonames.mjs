...
export function parseGeoNames(text, country) {
  const records = [], seen = new Set(); let rejected = 0, duplicates = 0;
  for (const line of text.replace(/^\uFEFF/, '').split(/\r?\n/)) {
    ...
    if (!Number.isFinite(latitude) || !Number.isFinite(longitude) || Math.abs(latitude) > 90 || Math.abs(longitude) > 180 || !/^\d{5}$/.test(f[1])) { rejected++; continue; }
...
export function makeDataset(imports, provenance) {
  ...
  return validateDataset({ schemaVersion: 1,
    meta: { title: `GeoNames postal localities — ${[...new Set(records.map(r => r.country))].sort().join(' + ')}`, coverage: 'country-files',
      ...
      changes: 'TSV converted to JSON; invalid coordinate/postcode rows excluded; exact duplicate rows removed; distances and locality reference points derived at query time.',
